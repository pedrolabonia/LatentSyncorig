...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        guidance_scale: float = Input(description="Guidance scale", ge=1, le=2.5, default=2),
        inference_steps: int = Input(description="Inference steps", ge=1, le=50, default=20),
        ...
        os.system(
            f"python -m scripts.inference --unet_config_path {config_path} --inference_ckpt_path {ckpt_path} --guidance_scale {str(guidance_scale)} --inference_steps {inference_steps} --video_path {video_path} --audio_path {audio_path} --video_out_path {output_path} --seed {seed}"
        )
